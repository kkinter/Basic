...
```sql
SELECT Country `나라`, COUNT(Country) `고객수`
FROM customers
GROUP BY Country
...
```sql 
SELECT COUNT(Country), Country, State
FROM customers
WHERE State in (
  SELECT State
FROM customers
